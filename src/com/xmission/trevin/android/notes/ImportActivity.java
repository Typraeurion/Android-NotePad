/*
 * $Id: ImportActivity.java,v 1.2 2015/03/28 20:30:43 trevin Exp trevin $
 ...
 * Revision 1.2  2015/03/28 20:30:43  trevin
 * Use the stored preference for the import type.
 *
 * Revision 1.1  2014/04/06 22:08:01  trevin
 * Initial revision
 *
...
public class ImportActivity extends Activity {
	...
	String fileName = Environment.getExternalStorageDirectory()
		    ...
		    + "/notes.xml";
	fileName = prefs.getString(NPREF_IMPORT_FILE, fileName);
	importFileName.setText(fileName);

	int importTypeIndex = prefs.getInt(NPREF_IMPORT_TYPE, 2);	// update
	importTypeList.setSelection(importTypeIndex);

	boolean importPrivate = prefs.getBoolean(NPREF_IMPORT_PRIVATE, true);
	...
	importFileName.addTextChangedListener(new TextWatcher () {
		...
		prefs.edit().putString(NPREF_IMPORT_FILE, s.toString()).commit();
	    }
	    @Override
	...
	importTypeList.setOnItemSelectedListener(
		new AdapterView.OnItemSelectedListener() {
			...
			prefs.edit().putInt(NPREF_IMPORT_TYPE, position).commit();
		    }
		});
	...
	importPrivateCheckBox.setOnCheckedChangeListener(
		new CompoundButton.OnCheckedChangeListener() {
			...
			prefs.edit().putBoolean(
				NPREF_IMPORT_PRIVATE, checked).commit();

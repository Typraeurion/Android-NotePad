...
implements SharedPreferences.OnSharedPreferenceChangeListener {
	...
	String whereClause = generateWhereClause();
        ...
        if ((selectedSortOrder < 0) ||
            ...
            prefs.edit().putInt(NPREF_SORT_ORDER, 0).commit();
	    selectedSortOrder = 0;
	}
	...
	if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
		...
		public Loader<Cursor> onCreateLoader(int id, Bundle args) {
			...
			public Cursor loadInBackground() {
			    ...
			    myCursor = getContentResolver().query(noteUri,
	...
	public void onItemSelected(AdapterView<?> parent, View v,
	    ...
	    Log.d(TAG, ".CategorySpinnerListener.onItemSelected(p="
		    + position + ",id=" + rowID + ")");
	    if (position == 0) {
		prefs.edit().putLong(NPREF_SELECTED_CATEGORY, -1).commit();
	    }
		...
		prefs.edit().putLong(NPREF_SELECTED_CATEGORY, rowID).commit();
	    }
    ...
    /** Look up the category corresponding to a category ID and select it. */
	...
	//	selectedSortOrder = 0;
		...
		Cursor itemCursor = managedQuery(noteUri,
			...
			NoteItem.USER_SORT_ORDERS[/*selectedSortOrder*/ 0]);
	...
	public void onClick(DialogInterface dialog, int which) {
	    Log.d(TAG, "CategoryDialogSelectionListener.onClick()");
	    if (which == 0) {
		// All
		prefs.edit().putLong(NPREF_SELECTED_CATEGORY, -1).commit();
		...
		prefs.edit().putLong(NPREF_SELECTED_CATEGORY, id).commit();
